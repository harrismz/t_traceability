<?php
	...
	*/
	...
	//	execute query
		$sql = "EXEC download_flash '{$valModelName}','{$valLotNo}','{$valSerialNo}','{$valDummySerial}'";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Insp_FLASH{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'MODEL, LOT NO / SERIAL NO, FLASH ID, INSPECT DATE, PCB SERIAL, JIG NO, JUDGE, INSP TIME, ARTFILENAME, NG CONTENT, M/C Name' . "\n";
		fwrite($fp,$headers);
