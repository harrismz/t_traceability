<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	// if ( $valRbCheck == 'DS' ){
		$sql = "EXEC download_fwdnDetail '','{$valModelName}','{$valLotNo}','{$valSerialNo}','{$valDummySerial}'";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Insp_FWDN_DETAIL{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'MODEL NAME, LOT NO / SERIAL NO, PCB SERIAL, FWDN ID, STEP, STEP DATA, MEASURE DATA, MEASURE, MCH NAME, INSP DATE' . "\n";
		fwrite($fp,$headers);
