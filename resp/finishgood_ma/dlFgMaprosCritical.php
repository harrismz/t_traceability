<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	if ( $valRbCheck == 'DS' ){
		$sql = "call downloadCriticalModelSerial ('{$valModelName}','{$valSerialNo}','{$valDummySerial}')";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Mapros_Critical{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'MODEL, LOTNO, DUMMY SERIAL, DUMMY PANEL, GUID MASTER, GUID TICKET, LCD SERIAL, PROCESS, JUDGE, EMP NO, PROCESS DATE' . "\n";
		fwrite($fp,$headers);
		...
		if ( $valRbCheck == 'MS' ){
			...
			$sql = "call downloadCriticalModelLotno ('{$valModelName}','{$valLotNo}')";
		}
	    $rs = $db->Execute($sql);
