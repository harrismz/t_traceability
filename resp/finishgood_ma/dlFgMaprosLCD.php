<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	if ( $valRbCheck == 'DS' ){
		$sql = "call downloadLCDModelSerial ('{$valModelName}','{$valSerialNo}','{$valDummySerial}')";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Mapros_LCD{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		if ( $valRbCheck == 'MS' ){
			...
			$sql = "call downloadLCDModelLotno ('{$valModelName}','{$valLotNo}')";
		}
	    $rs = $db->Execute($sql);
