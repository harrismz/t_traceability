<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	if ( $valRbCheck == 'DS' ){
		$sql = "call downloadMasterModelSerial ('{$valModelName}','{$valSerialNo}','{$valDummySerial}')";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Mapros_Master{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'MODEL, LOTNO, DUMMY SERIAL, DUMMY PANEL, GUID MASTER, STATUS, PROCESS, JUDGE, EMP NO, PROCESS DATE' . "\n";
		fwrite($fp,$headers);
		
		while(!$rs->EOF)
		{
			if(strlen($rs->fields['8']) > 5 && strlen($rs->fields['8']) < 9 ){
				$empno = substr($rs->fields['8'], 2, 5);
			}
			else { 
				$empno = $rs->fields['8'];
			} 
		...
		if ( $valRbCheck == 'MS' ){
			...
			$sql = "call downloadMasterModelLotno ('{$valModelName}','{$valLotNo}')";
		}
	    $rs = $db->Execute($sql);
		...
		$fp = fopen("php://output", "w");
		$headers = 'SERIAL NO, MODEL, LOTNO, DUMMY SERIAL, DUMMY PANEL, GUID MASTER, STATUS, PROCESS, JUDGE, EMP NO, PROCESS DATE' . "\n";
		fwrite($fp,$headers);
