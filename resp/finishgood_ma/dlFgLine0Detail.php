<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	// if ( $valRbCheck == 'DS' ){
		$sql = "EXEC download_line0Detail '{$valModelName}','{$valSerialNo}','{$valDummySerial}'";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Insp_LINE0_DETAIL{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'ID LINE 0, ROW NUMBER, STEP, STEP DATA, MEASURE DATA, MEASURE, M/C NAME, DATE INSPECT' . "\n";
		fwrite($fp,$headers);
