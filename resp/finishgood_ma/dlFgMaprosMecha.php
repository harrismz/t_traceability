<?php
	...
	*/
	...
	$valDummySerial	= trim(@$_REQUEST["ds"]);
		...
		//	execute query
	if ( $valRbCheck == 'DS' ){
		$sql = "call downloadMechaModelSerial ('{$valModelName}','{$valSerialNo}','{$valDummySerial}')";
		$rs = $db->Execute($sql);
		...
		$fname = "Traceability_Finishgood_Mapros_Mecha{$fmodel}{$flotno}{$fserial}{$fdummy}.csv";
		...
		$headers = 'MODEL, LOTNO, DUMMY SERIAL, DUMMY PANEL, GUID MASTER, GUID TICKET, MECHA SERIAL, PROCESS, STATUS, JUDGE, EMP NO, PROCESS DATE' . "\n";
		fwrite($fp,$headers);
		...
		if ( $valRbCheck == 'MS' ){
			...
			$sql = "call downloadMechaModelLotno ('{$valModelName}','{$valLotNo}')";
		}
	    $rs = $db->Execute($sql);
		...
		$fp = fopen("php://output", "w");
		$headers = 'SERIAL NO, MODEL, LOTNO, DUMMY SERIAL, DUMMY PANEL, GUID MASTER, GUID TICKET, MECHA SERIAL, PROCESS, STATUS, JUDGE, EMP NO, PROCESS DATE' . "\n";
		fwrite($fp,$headers);
