<?php
    ...
    $rs_tot = $db->Execute("select count(*) from show_part_install('{$model}', '{$serial}', '{$proddate}', '{$src_cat}')");
    $totalcount = $rs_tot->fields['0'];
	
    $rs    = $db->Execute("select first $limit skip $start * from show_part_install('{$model}', '{$serial}', '{$proddate}', '{$src_cat}') order by jobdate, jobtime desc");
    $return = array();
